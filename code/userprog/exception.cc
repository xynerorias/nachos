...
void ExceptionHandler(ExceptionType which)
{
  ...
  case SyscallException:
  {
    ...
    case SC_PutString:
    {
      ...
      copyStringFromMachine(uaddr, buf, MAX_STRING_SIZE);
    ...
    case SC_GetString:
    {
      int uaddr = machine->ReadRegister(4); // r4=s(adresse MIPS)
      int n = machine->ReadRegister(5);     // r5=n(taille tampon user)
      if (n <= 0)
      {
        UpdatePC();
        return;
      }
      char *buf = new char[n];                      // tampon max n
      consoledriver->GetString(buf, n);             // lit depuis la console
      copyStringToMachine(uaddr, buf, (unsigned)n); // recopie -> user
      delete[] buf;
    ...
    case SC_ThreadCreate:
    {
      ...
      DEBUG('x', "ThreadCreate called with f=%d arg=%d\n", f, arg);

      int threadId = do_ThreadCreate(f, arg);
      machine->WriteRegister(2, threadId);

      UpdatePC();
      DEBUG('x', "ThreadCreate syscall returning %d\n", threadId);
      return;
    }
    case SC_ThreadExit:
    {
